...
using QueueStrategy = Strategy_t<queue<Bill>>;
using StackStrategy = Strategy_t<stack<Bill>>;
...
template<typename T>
auto & peek( stack<T> & container )
{ return container.top(); }

template<typename T>
auto & peek( queue<T> & container )
...
template<class Strategy>
Strategy_t<Strategy>::Strategy_t( const string & name,
                                  double starting, double salary )
...
bool Strategy_t<Strategy>::payBills(const unsigned int & currMonth,
  ...
  while( allBills_.size() > 0 ) {
    ...
    //  int days_overdue = someBill.daysOverdue(currMonth, currDay);
...
template<class Strategy>
void Strategy_t<Strategy>::readFile( const string & filename )
{
